...
def token_lookup():
    ...
    """
    # TODO: Implement Function
    punct={'.':'||Period||',',':'||Comma||','"':'||Quotation_Mark||',';':'||Semicolon||',
            '!':'||Exclamation_Mark||','?':'||Question_Mark||','(':'||Left_Paranthesis||',')':'||Right_Parenthesis||',
            '--':'||Dash||','\n':'||Return||'}
    return punct
...
def build_nn(cell, rnn_size, input_data, vocab_size, embed_dim):
    ...
    #print(em)
    output,final = build_rnn(cell,em)
...
def get_batches(int_text, batch_size, seq_length):
    ...
    input_batch = np.split(inputs.reshape(batch_size, -1), n_batches, 1)
    output_batch = np.split(outputs.reshape(batch_size, -1), n_batches, 1)


    return np.array(list(zip(input_batch, output_batch)))
...
def get_tensors(loaded_graph):
    ...
    return inputtensor, init_state, final_state, probs
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    # TODO: Implement Function
    next_word=np.random.choice(len(probabilities),10,p=probabilities)
    next_word.sort()
    return int_to_vocab[next_word[9]]
